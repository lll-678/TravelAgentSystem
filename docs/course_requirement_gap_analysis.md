...
| 室内导航 | Covered for demo | Existing `综合教学楼` remains as a fallback. Stage 38 adds `中国科学技术馆主展厅` with B1-5F, exhibition halls, services, elevators/stairs/escalators, Dijkstra routing, accessible-mode filtering, and frontend start/end selection. The floor plan is a schematic graph based on public venue-guide structure, not copied CAD. |
