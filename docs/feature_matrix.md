...
| Routing | Place-name route endpoint selection | `GET /api/v1/search/places`, `POST /api/v1/routes/plan` with `start_place_id/end_place_id` | RoutePlannerPage | `destinations`, `buildings`, `facilities`, `map_nodes`, `map_edges` | tested: place ID route API + frontend typecheck |
